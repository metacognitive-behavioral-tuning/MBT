...
# qa.evaluation across the full paper-table model matrix on QA validation.
# Judge: gemma-4-31b-it. EM/substring/F1/llm_as_judge per (dataset, variant).
# 17 variants per Qwen3-{0.6B,1.7B,4B} + gpt-oss-120b-high base = 52 entries × 3 datasets.
# Path conventions exactly mirror scripts/data_collection.py EXPERIMENTS.
...
uv run mbt \
    ...
    --api-name "vllm.chat" \
    --api-config '{"model_name": "gemma-4-31b-it", "model_kwargs": {"config": "configs/vllm/gemma-4-31b-it.yaml"}, "request_kwargs": {"temperature": 0.6, "top_p": 0.95, "n": 1, "extra_body": {"top_k": 20}}, "num_threads": 1024, "client_timeout": 60}' \
