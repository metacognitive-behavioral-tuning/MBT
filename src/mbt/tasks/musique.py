...
class Task:
    """
    Task wrapper for the MuSiQue multi-hop QA dataset.

    This class handles data loading, preprocessing, and prompt construction for the MuSiQue dataset.
    It supports various prompting strategies including standard QA, metacognitive prompting,
    and solution generation (naive, standard, or metacognitive) for creating training data or baselines.

    Args:
      task_config (dict): A configuration dictionary containing the following keys:
        - dataset_name (str, optional): The Hugging Face dataset path. Defaults to "dgslibisey/MuSiQue".
...
def format_columns(example: dict, idx: int) -> dict:
    ...
    answer_aliases = example.pop("answer_aliases")
    return {"sample_id": idx + 1, "metadata": example, "question": question, "answer": answer, "answer_aliases": answer_aliases}
...
def build_prompt(example: dict, idx: int, metacognitive_prompt: bool, solution: bool, mbt_s: bool) -> dict:
    ...
    variables["context"] = "\n".join([f"Document [{doc['idx']}] (Title: {doc['title']}) {doc['paragraph_text']}" for doc in example["metadata"]["paragraphs"]])
